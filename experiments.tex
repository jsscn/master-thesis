...
% TODO re-enable if window width experiments succeeded
...
Finding the association rules from parallel episodes in \emph{trains} using the fixed-window frequency (Figure~\ref{fig:runtimes-rules-trains-900-parallel}) started to take significant amounts of time for the lowest frequency thresholds (largest quantities episodes produced). However, given that the graphs in the log--log plot are mostly linear, we would judge the complexity to be polynomial.

On the contrary, for association rules from serial episodes in \emph{trains}, the graph does curve upwards somewhat towards the end (Figure~\ref{fig:runtimes-rules-trains-900-parallel}), possibly due to the fact that the number of subepisodes is exponential in the size of an episode.
...
The window width places an upper bound on the maximal distance between two events that are part of an occurrence of an episode. Consequently this also limits the number of candidate patterns. We know that the number of candidate episodes is exponential in the number of nodes. Luckily the window width usually doesn't need to be dramatically high, such that it doesn't become a real problem. Still, it can be good to know how the window width affects the runtimes.

From our previous experiments generating episodes (Section~\ref{sec:performance-episodes}), for each combination of episode class and frequency measure, we decided on a threshold which finishes in a reasonable amount of time for the window width we chose for the respective datasets --- 15 for \emph{abstract} and 900 for \emph{trains}. Then we varied the window width --- 3, 6, 9, 12, 15 for \emph{abstract} and 180, 360, 540, 720, 900 for \emph{trains}. Because of this, the absolute values are of no importance; only the progression.

The results for \emph{abstract} are shown in Figure~\ref{fig:abstract-window-width}. For parallel episodes, the fixed-window frequency seems to be affected the most. We saw that for lower thresholds, the fixed-window frequency produced large amounts of large episodes. The weighted-window frequency didn't seem to be affected much at all. This is likely due to the fact that it naturally produces fewer large episodes, given its preference for small minimal windows.

Figure~\ref{fig:trains-window-width} shows the results for \emph{trains}. Interestingly, the runtimes for parallel episodes and fixed-window frequency remain quite stable until a window width of 720, after which it jumps upwards significantly. For some reason the number of frequent episodes grew over eight times as large. In the same interval, the runtimes for the disjoint-window frequency increased quite a bit, whereas it progressed more stable in the other graphs. Perhaps the sparse nature of the \emph{trans} dataset gives rise to significantly more The weighted-window frequency is also noticeably less stable than with \emph{abstract}.
...
\subsection{Performance comparison closed episode miner}
...
We think that there must be opportunities left for improving the performance of our implementation. Although we tried to do our best, we are not experts at optimizing code for speed. Often our algorithm had to deal with millions of candidates, and combined with the long sequences, optimization was not an easy task.

Doesn't mean that our implementation is unusably slow.
