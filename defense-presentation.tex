...
\pgfplotsset{legend style={fill opacity=0.7,text opacity=1,nodes={scale=0.75}}}
...
\item $ \nexists [c, d) \subset [a, b) : \boldsymbol{s}[a, b) \text{ covers } \alpha $
...
\draw [thick] (-4.5,-7pt) -- ++(0,-3pt) -- ++(1.4,0) node [midway,below] {$ \dfrac13 $} -- ++(0,3pt);

\draw [thick] (1.5,-7pt) -- ++(0,-3pt) -- ++(2.9,0) node [midway,below] {$ \dfrac16 $} -- ++(0,3pt);
...
\# & quantile-based cohesion & $ C_q $ & \text{supp.} \\
\hline
...
\end{frame}
\begin{frame}
\frametitle{Runtimes \emph{abstract}}
...
\iffalse
\begin{frame}
\frametitle{Runtimes \emph{tolstoy}}
...
\fi
\begin{frame}
\frametitle{Runtimes \emph{trains}}
