...
Next, we define patterns, named \emph{episodes} in sequence pattern mining, and cover the classes of episodes we will be mining. From these episodes we can construct \emph{association rules}, which can uncover correlations between the different events in an event sequence.

Lastly, we define several \emph{interestingness measures} proposed by different researchers, which quantify how interesting an episode or an association rule is in a sequence.
...
Given a pattern and a sequence, we would like to quantify how interesting the pattern is. One way to think of \emph{interesting} is in terms of how often it occurs in the sequence. While other ways exist [TODO cite (?)], we will mainly focus on \emph{frequency measures}, which we'll discuss in section~\ref{sec:interestingness-measures}. First we formally define patterns.
...
Figure~\ref{fig:occurrences} shows an occurrence of two episodes---one parallel, one serial---in the example sequence from figure~\ref{fig:event-sequence}.
...
Now that we have defined patterns on event sequences, we would like to be to know how interesting a pattern is in a sequence.

\section{Association rules}

We've touched upon the fact that we want to quantify how interesting a pattern is. Besides quantifying a single episode, it might also be desirable to know which patterns are correlated. \emph{Association rules}~\citep{mannila1997discovery} allow us to do so, similarly to association rules in itemset mining.
...
Intuitively, An association rule $ \alpha \Rightarrow \beta $ can be read as $ \alpha $ \emph{leads to} $ \beta $. More precisely, an occurrence of $ \alpha $ leads to an occurrence of $ \beta $. Or, if we find an occurrence of $ \alpha $, then we'll find an occurrence of $ \beta $ \emph{nearby}.

If we state that $ \alpha \Rightarrow \beta $, we must be \emph{confident} stating it. So, given an association rule and a sequence, we want to express \emph{how} confident we are about the association rule, and define measures to quantify that. We will cover the measures in section~\ref{sec:interestingness-measures}, together with the frequency measures of episodes.

\section{Interestingness measures}
\label{sec:interestingness-measures}
...
\subsection{Minimal windows}

Another approach towards defining the frequency of an episode in a sequence is to use minimal windows. The idea was put forward in~\citep{mannila1997discovery}, and was expanded in~\cite{laxman2005discovering} and \citep{laxman2005discovering}.
...
Minimal windows of an episode can still overlap, however. As a very simple example consider the sequence $ a\;b\;a $ and episode $ \{ a, b \} $. Then both $ [1, 3) $ and $ [2, 4) $ are minimal windows of $ \{ a, b \} $. They cannot overlap arbitrarily, though: given two windows for an episode $ [a, b) $ and $ [c, d) $, (without loss of generality) it holds that $ a < c \Leftrightarrow b < d $. If this wasn't the case, then one would be contained in the other, and by definition~\ref{def:minimal-window} the other would not be a minimal window, which contradicts the initial assumption.
