...
\EndFor
\end{algorithmic}
By initializing $ j $ to $ \mathcal{F}_l \text{.block\_start}[i] $, $ j $ can be less than $ i $, alleviating the constraint on parallel episodes.
...
\For{$ \text{start} \gets T_s - \rho + 1 $ to $ T_e - 1 $} % TODO check if starting at correct position
    ...
    \ForAll{events $ (A, t) $ in $ s $ such that $ t = \text{start} + \rho - 1 $}
        \State $ \text{in\_window}[A] \text{.push}(t) $ \label{alglin:rec-par-mwi:push-timestamp}
        \ForAll{$ \alpha $ in $ \text{contains}(A) $}
            \State $ \text{consider\_max}(\alpha, A) \gets \text{consider\_max}(\alpha, A) + 1 $
            \If{{\footnotesize $ \forall A $ in $ \alpha : \min(| \text{in\_window}(A) |, \text{consider\_max}(\alpha, A)) \geq \text{num\_needed}(\alpha, A) $ }} \label{alglin:rec-par-mwi:check-min-window-satisfied}
                \LineComment{Minimal window found; determine the start}
                \State $ Q \gets \text{in\_window}(A) $
                \State $ \text{window\_start} \gets \min\{ t \mid A $ in $ \alpha \wedge t = Q[| Q | - \text{num\_needed}(\alpha, A) + 1] \} $
                \ForAll{$ A $ in $ \alpha $}
...
Recognizing parallel episodes with the minimal-windows frequency measure is slightly more complex than with the fixed-window frequency. Algorithms for recognizing parallel and serial episodes were presented in~\cite{laxman07}. However, those algorithms only determine the disjoint-window frequency. Because we also want to be able to determine the minimal-window confidence of association rules, not only do we need the total number of minimal windows; we also need the minimal windows themselves.

% Our algorithm wasn't based on Laxman's.

Thanks to the requirement that all minimal windows have a width of at most $ \rho $, we can keep using a sliding window of fixed width, just like the algorithms for the fixed-window frequency. Then only the part of the sequence included in the sliding window needs to be considered at any time, and any minimal window we find will be a subwindow of the current sliding window. We'll still be scanning the front of the sliding window for new events as before. Now, when a new event $ (A, t) $ comes in, we need to answer the following questions regarding an episode $ \alpha $ containing $ A $:
\begin{enumerate}
...
Algorithm~\ref{alg:rec-par-mwi} keeps a queue for each event type $ A $, which contains the timestamps of the events of type $ A $ currently in the sliding window. The queues are referred to by \textbf{in\_window} in the pseudocode. When a new event $ (A, t) $ comes in, $ t $ is pushed onto the queue for event type $ A $ (line~\ref{alglin:rec-par-mwi:push-timestamp}). Then, for each episode $ \alpha $ containing $ A $, it is determined whether we have a new minimal window, by the number of timestamps in each queue for the event types in $ \alpha $, and, as we mentioned before, excluding the events at the start of the most recently found minimal window. The exclusion of these events is realized by \textbf{consider\_max}.

If we have determined that there are enough events in consideration to form a new minimal window (line~\ref{alglin:rec-par-mwi:check-min-window-satisfied}), then the queues allow us to find the start of the window. If there are $ n $ nodes of type $ A $ in $ \alpha $, then we look at the $ n $-th most recently added element to $ A $'s queue. The minimum of the timestamps for each $ A $ in $ \alpha $ is the start of the minimal window.

In figure~\ref{fig:rec-par-mwi-example}, all of the minimal windows covering $ \{ a, b \} $ are drawn.
As shown before, minimal windows can overlap. This means that we can't just look for a minimal window and, once found, ``start over'' entirely.
...
Once an episode has reached state $ | \alpha | $, a minimal window covering the episode starts at the point where the automaton was initialized, and ends at the front of the current sliding window. So the displacement of the window after reaching this state does not need to be kept track of anymore.
...
\subsection{Determining the weighted-window frequency}
