...
\subsection{Determining the fixed-window frequency for parallel episodes}
...
\subsection{Determining the fixed-window frequency for serial episodes}
...
\subsubsection{Correcting the original algorithm presented in~\citep{mannila1997discovery}}
...
\subsection{Finding all minimal windows of parallel episodes}
...
\subsection{Finding all minimal windows of serial episodes}
...
\For{$ \text{start} \leftarrow T_s - \rho + 1 $ to $ T_e $} \label{alglin:rec-ser-mwi:iterate-sequence} \Comment{Recognition}
    ...
    \EndFor
    \ForAll{$ ( \alpha, j, t ) \in \text{transitions} $}
        \If{$ \alpha \text{.initialized}[j] \neq \text{\textit{uninitialized}} $} \label{alglin:rec-ser-mwi:added-line-1} \label{alglin:rec-ser-mwi:transition-begin} \label{alglin:rec-ser-mwi:transition-begin}
...
\subsection{Determining the minimal-window confidence}
...
\subsubsection{Correcting the original algorithm presented in~\citep{cule2014marbles}}
