...
Finding the minimal windows of parallel episodes is slightly more complex than determining the fixed-window frequency. Algorithms for finding the disjoint-window frequency of parallel and serial episodes were presented in~\cite{laxman07}. However, we also want to be able to determine the minimal-window confidence of association rules, and so we also need the minimal windows themselves (including overlapping windows).
...
The disjoint-window frequency of an episode is the largest possible number of non-overlapping minimal windows of the episode.

If we just needed the disjoint-window frequency, it is possible to recognize only disjoint minimal windows in the sequence. However, to determine the minimal-window confidence and the weighted-window confidence of association rules, we need all minimal windows. Hence, we use a procedure that determines the disjoint-window frequency based on a list of all minimal windows of an episode.
...
Then, in lines~\ref{alglin:wwi:begin-wwi-calculation} through~\ref{alglin:wwi:end-wwi-calculation}, the weighted-window frequency is determined through a backwards pass over the minimal windows. Each $ c_i $ is the optimal choice of windows $ i $ through $ n $. If we know all $ c_j $ with $ j > i $, then $ c_i $ can be calculated as
\begin{align*}
