...
Each episode has its own automaton, which consists of $ | \alpha | $ states: each state corresponds to a node in the episode. A state can be represented by the corresponding node's index in the array representation of the episode; state 1 referring to the first node in the topological ordering of the episode graph, and so on. Then an instance of the automaton for $ \alpha $ being in a state $ j $ denotes that the episode has been recognized up to (and including) the $ j $-th node. When in state $ j $ and upon encountering an event of which the type corresponds to the type of the $ (j + 1) $-th node of the episode, the instance will transition to state $ j + 1 $.
...
\node (marktxt) [above=0.5cm of n2-0] {mark this timestamp};
\draw[->] (marktxt) -- (n2-0);
...
\node (outofwindowtxt) [above=0.5cm of n3-3] {calculate difference with this timestamp};
\draw[->] (outofwindowtxt) -- (n3-3);

\end{tikzpicture}

\caption{Illustration recognition of serial episode $ B \to A \to C $. Black dots are timestamps which don't contain any event types in $ B \to A \to C $.}
...
After processing the state transitions, the events which are no longer in the window, are dropped. This entails looking up all automata which were instantiated at timestamp $ (\text{start} - 1) $ using the \emph{begins\_at} data structure (line~\ref{alglin:rec-ser-fwi:cleanup-iteration-begin} and further):

\begin{enumerate}
\item If the automaton instance was in state $ | \alpha | $, the episode occurred within the window until now. When this automaton instance first came into the sliding window, the timestamp was stored in $ \alpha \text{.in\_window} $, and so the frequency count of the episode can be updated by adding $ (\text{start} - \alpha \text{.in\_window}) $.
