...
\For{$ \text{start} \gets T_s - \rho + 1 $ to $ T_e - 1 $} % TODO check if starting at correct position
    ...
    \ForAll{events $ (A, t) $ in $ s $ such that $ t = \text{start} + \rho - 1 $}
        \State $ \text{in\_window}[A] \text{.push}(t) $
        \ForAll{$ \alpha $ in $ \text{contains}(A) $}
            \If{{\footnotesize $ \forall A $ in $ \alpha : \min(| \text{in\_window}(A) |, \text{consider\_max}(\alpha, A)) \geq \text{num\_needed}(\alpha, A) $ }}
                ...
                \State $ \text{window\_start} \gets \min\{ t \mid \forall A $ in $ \alpha : t = Q[| Q | - \text{num\_needed}(\alpha, A) + 1] \} $
                \ForAll{$ A $ in $ \alpha $}
...
Thanks to the requirement that all minimal windows be smaller than a given window width $ \rho $, we can keep using a sliding window of fixed width, just like the algorithms for the fixed-window frequency. In that way only the part of the sequence included in the sliding window needs to be considered at any time, and so any minimal window we find will be a subwindow of the current sliding window. We'll still be scanning the front of the sliding window for new events, as before. Now, when a new event (A, t) comes in, we need to answer the following questions regarding an episode $ \alpha $ containing $ A $:
\begin{enumerate}
\item With this new event in the window, is there a minimal window $ [a, \text{start} + \rho) $ covering $ \alpha $ which ends at the front of the current sliding window? ($ b = \text{start} + \rho $)
\item If so, where does the minimal window start: what is $ a $?
\end{enumerate}

Since minimal windows will often be smaller than the fixed-width sliding window, a previously recognized minimal window may still be within the sliding window several timestamps after it has been recognized. Therefore we cannot just rely on the number of events of each type currently in the sliding window, as we did in algorithm~\ref{alg:rec-par-fwi}---we need some way of excluding that previous minimal window from consideration. Any newly discovered minimal window has to start \emph{after} the start of the previous one.

Algorithm~\ref{alg:rec-par-mwi} keeps a queue for each event type $ A $ containing the timestamps of the events of type $ A $, referred to by \textbf{in\_window} in the pseudocode. When a new event $ (A, t) $ comes in, $ t $ is pushed onto the queue for event type $ A $. Now, for each episode $ \alpha $ containing $ A $, it is determined whether we have a new minimal window, by:
\begin{enumerate}
\item the number of timestamps in each queue for the event types in $ \alpha $, and
\item word this better some other time % TODO
\end{enumerate}
