...
Algorithm~\ref{alg:episodes-top-level} describes the high-level procedure to find all frequent episodes that are frequent in an event sequence $ s $, for a certain class of episodes (parallel or serial) and given window width \emph{win} and frequency threshold \emph{min\_fr}. It is a breadth-first algorithm, that is, all frequent episodes of size $ l $ are computed before those of size $ l + 1 $.
...
\For{$ \text{start} \gets T_s - \text{win} + 1 $ to $ T_e - 1 $} % TODO check if starting at correct position
    ...
    \ForAll{events $ (A, t) $ in $ s $ such that $ t = \text{start} + \text{win} - 1 $}
        ...
        \ForAll{$ \alpha $ in $ \text{contains}(A) $}
            \If{$ \forall A $ in $ \alpha : min(| \text{events\_in\_window}(A) |, \text{consider\_max}(\alpha, A)) \geq \text{num\_needed}(\alpha, A) $}
...
\caption{Remove infrequent episodes from a collection of candidates $ \mathcal{C} $ for which \emph{freq\_count} is known. \\
Input: A list of candidates $ \mathcal{C} $, including their \emph{block\_start} values, and their \emph{freq\_count} values with respect to some sequence, and a minimum frequency threshold \emph{min\_fr}. \\
Output: Those episodes in $ \mathcal{C} $ which are frequent, along with consistent \emph{block\_start} values.
}
