...
\draw (n3-2.south) -- ++(0,-6pt) -- node (numwindowsindicator) [near start,below right,align=left] {determine minimal window \\ using \emph{num\_needed}, $ Q $, and \emph{start}} ++(3.5*\interdotdistance,0) -- ++(0,6pt);
...
\node [right=9em of dr5,align=left] {new $ a $ entered; \\ enough events in \\ consideration again};

\slidingwindowthingy{-8}{6}
\draw (7.5*\interdotdistance,-8-0.5*\slidingwindowheight) -- ++(-5*\interdotdistance,0) -- ++(0,\slidingwindowheight) -- ++(5*\interdotdistance,0);
...
\caption{Recognition of parallel episode $ \alpha = \{ a, a, b \} $ for finding its minimal windows in the sequence. Black dots are timestamps which don't contain $ a $ or $ b $.}
