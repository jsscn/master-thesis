...
\ForAll{$ \beta \in \mathcal{F}(\boldsymbol{s}, \rho, \text{min\_fr}) $}
    \ForAll{$ \alpha \subset \beta $} \Comment{Algorithm [TODO write thingies]}
        \If{$ c(\alpha \Rightarrow \beta) \geq \text{min\_conf} $} \label{alglin:association-rules-top-level:compute-confidence}
